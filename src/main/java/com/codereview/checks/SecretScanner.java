...
import com.codereview.util.SeverityMapper;
                ...
                for (Pattern p : PATTERNS) {
                    ...
                    if (m.find()) {
                        findings.add(new Finding(
                                f.getAbsolutePath(),
                                lineNo,
                                SeverityMapper.forSecret(m.group(0)),   // ⬅ Correct
                                "Secret",
                                "Possible hardcoded secret: " + m.group(0).trim()
                        ));
                    }
                }
                        ...
                        if (isPotentialSecretToken(token)) {
                            ...
                            if (ent > 4.0) {
                                ...
                                findings.add(new Finding(
                                        f.getAbsolutePath(),
                                        lineNo,
                                        SeverityMapper.forSecret(token),   // ⬅ FIXED (NO m.group())
                                        "Secret",
